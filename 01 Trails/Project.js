        ...
        for (let j = 0; j < map.pixelMap[i].length; j++) {
            for (let x = 0; x < scale; x++) {
                for (let y = 0; y < scale; y++) {
                    writeColor(img, i * scale + x, j * scale + y, map.pixelMap[i][j][0], map.pixelMap[i][j][1], map.pixelMap[i][j][2], map.pixelMap[i][j][3]);
                }
            }
        }
    }
...
function generateAgents() {
    ...
    let angle = 0;
    for (let i = 0; i < noOfAgents; i++, angle += incr) agents[i] = new Agent(centerX, centerY, 1, angle);
}
